...
[CreateAssetMenu(fileName = "WindowsDatasAsset", menuName = "DataAsset/WindowsDatasAsset")]
public class WindowsAsset : BaseAsset<WindowsData>
{
    [Header("Config")]
    ...
    [ButtonMethod]
    public List<WindowsData> GetNotUnlockedSkin()
    {
        List<WindowsData> allWindows = new List<WindowsData>();
        foreach (var item in list)
        {
            allWindows.Add(item);
        }
        allWindows.RemoveAt(0);
        return allWindows;
    }
    [ButtonMethod]
...
[Serializable]
public class WindowsData : SaveData
{
    [Header("GameData")]
