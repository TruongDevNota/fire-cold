...
using MyBox;
[CreateAssetMenu(fileName = "CeillingDataAsset", menuName = "DataAsset/CeillingDataAsset")]
public class CeillingAsset : BaseAsset<CeillingData>
{
    [Header("Config")]
    ...
    [ButtonMethod]
    public List<CeillingData> GetNotUnlockedSkin()
    {
        List<CeillingData> allCeillings = new List<CeillingData>();
        foreach (var item in list)
        {
            allCeillings.Add(item);
        }
        allCeillings.RemoveAt(0);
        return allCeillings;
    }
    [ButtonMethod]
...
[Serializable]
public class CeillingData : SaveData
{
    [Header("GameData")]
