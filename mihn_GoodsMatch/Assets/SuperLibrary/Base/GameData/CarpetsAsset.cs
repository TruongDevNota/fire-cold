...
using MyBox;
[CreateAssetMenu(fileName = "CarpetsDataAsset", menuName = "DataAsset/CarpetsDataAsset")]

public class CarpetsAsset : BaseAsset<CarpetData>
{
    [Header("Config")]
    ...
    [ButtonMethod]
    public List<CarpetData> GetNotUnlockedSkin()
    {
        List<CarpetData> allCarpets = new List<CarpetData>();
        foreach (var item in list)
        {
            allCarpets.Add(item);
        }
        allCarpets.RemoveAt(0);
        return allCarpets;
    }
    [ButtonMethod]
...
[Serializable]
public class CarpetData : SaveData
{
    [Header("GameData")]
