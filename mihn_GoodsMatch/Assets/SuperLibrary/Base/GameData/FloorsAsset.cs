...
[CreateAssetMenu(fileName = "FloorDataAsset", menuName = "DataAsset/FloorDataAsset")]
public class FloorsAsset : BaseAsset<FloorData>
{
    [Header("Config")]
    ...
    [ButtonMethod]
    public List<FloorData> GetNotUnlockedSkin()
    {
        List<FloorData> allFloors = new List<FloorData>();
        foreach (var item in list)
        {
            allFloors.Add(item);
        }
        allFloors.RemoveAt(0);
        return allFloors;
    }
    [ButtonMethod]
...
[Serializable]
public class FloorData : SaveData
{
    [Header("GameData")]
